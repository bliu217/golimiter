...
local granted = 0
if requested > 0 and tokens > 0 then
  granted = math.min(requested, tokens)
  tokens = tokens - granted
end

local reset_time_seconds = 0
if granted < requested and refill_rate > 0 then
  reset_time_seconds = math.ceil((requested - granted) / refill_rate)
end
...
return {tostring(granted), tostring(tokens), tostring(reset_time_seconds)}
